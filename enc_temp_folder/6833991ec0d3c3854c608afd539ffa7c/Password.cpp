...
System::String^ CheckPassword(System::String^ toCheck, System::Windows::Forms::Label^ label) {			// реалізація функціі для перевірки паролю на складність
	...
	if (small_letter && !capital_letter && !number && !special_char || // строка содержит только маленькие буквы
		!small_letter && capital_letter && !number && !special_char || // строка содержит только большие буквы
		!small_letter && !capital_letter && number && !special_char || // строка содержит только цифры
		!small_letter && !capital_letter && !number && special_char) { // строка содержит только специальные символы
